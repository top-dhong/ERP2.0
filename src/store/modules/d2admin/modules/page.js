...
export default {
  ...
  actions: {
    ...
    open({ state, commit, dispatch }, { name, params, query }) {
      console.log('page open', { name, params, query })
      return new Promise(async resolve => {
        // 已经打开的页面
        ...
        const pageOpend = opened.find((page, index) => {
          // let same = page.name === name
          // if (page.query != null && page.query.BillID != null) {
          //   same = page.name === name && page.query.BillID == query.BillID
          //   console.log(page.query.BillID);
          // }
    ...
    close({ state, commit, dispatch }, { tagName, vm }) {
		console.log(1,{ state, commit, dispatch }, { tagName, vm })
      //------------------
      if (tagName.includes('Temp')) {
        let opened = JSON.parse(util.cookies.get("glb_opened"))
        let name = tagName.split('').reverse().join('');
        name = name.substr(0,name.indexOf('-'));
        name = name.split('').reverse().join('');
        let idx = opened.indexOf(name);  
        if (idx >=0) {opened.splice(idx,1);}
        util.cookies.set("glb_opened", opened)
        console.log('成功移除页面=',tagName)
