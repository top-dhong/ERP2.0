  ...
  <d2-container>
    <template slot="header">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>{{flowTypeName||'公司发文（地产）'}}</el-breadcrumb-item>
      </el-breadcrumb>
    </template>
    <div>
      <el-row>
			...
			<el-row>
			  <el-col :span="8">
			    <el-form-item
			      label="发文分类："
			      class="te-item"
			      prop="flowBaseData.text1"
			      :rules="[{ required: true, message: '发文分类不能为空',trigger:'blur'}]"
			    >
			  			<el-select v-model="flowData.flowBaseData.text1" placeholder="请选择" style="width:100%">
			  		      <el-option
			  		        v-for="item in text1Options"
			  		        :key="item.value"
			  		        :label="item.label"
			  		        :value="item.value">
			  		      </el-option>
			  ...
			  <el-col :span="6">
			    <el-form-item
			      label="紧急程度："
			      class="te-item"
			      prop="flowBaseData.text2"
			      :rules="[{ required: true, message: '紧急程度不能为空',trigger:'blur'}]"
			    >
			  			<el-select v-model="flowData.flowBaseData.text2" placeholder="请选择" style="width:100%">
			  		      <el-option
			  		        v-for="item in text2Options"
			  		        :key="item.value"
			  		        :label="item.label"
			  		        :value="item.value">
			  		      </el-option>
			  ...
			  <el-col :span="6">
			    <el-form-item
			      label="等级密级："
			      class="te-item"
			      prop="flowBaseData.text3"
			      :rules="[{ required: true, message: '等级密级不能为空',trigger:'blur'}]"
			    >
			  			<el-select v-model="flowData.flowBaseData.text3" placeholder="请选择" style="width:100%">
			  		      <el-option
			  		        v-for="item in text3Options"
			  		        :key="item.value"
			  		        :label="item.label"
			  		        :value="item.value">
			  		      </el-option>
			  ...
			  <el-col :span="3">
			    <el-form-item
			      ...
			      :rules="[{ required: true, message: '是否人事任命不能为空',trigger:'blur'}]"
			    >
			      <el-checkbox v-model="flowData.flowCoreData.text4"></el-checkbox>
			    </el-form-item>
			  </el-col>
			</el-row>
			<el-row>
				<el-col :span="12">
					<el-form-item label="发布标题：" class="te-item" prop="flowCoreData.text6" :rules="[{ required: true, message: '发布标题不能为空',trigger:'blur'}]">
						<el-input v-model="flowData.flowCoreData.text6" clearable></el-input>
					</el-form-item>
				</el-col>
				<el-col :span="12">
					<el-form-item label="发布编号：" class="te-item" prop="flowCoreData.text7" :rules="[{ required: true, message: '发布编号不能为空',trigger:'blur'}]">
						<el-input v-model="flowData.flowCoreData.text7" clearable></el-input>
					</el-form-item>
				</el-col>
			</el-row>
			<el-row>
				<el-col :span="24">
				  <el-form-item
				    label="可阅读者："
				    class="te-item"
				    prop="flowCoreData.text8"
				    :rules="[{ required: true, message: '可阅读者不能为空',trigger:'blur'}]"
				  >
				    <el-checkbox v-model="flowData.flowCoreData.text8">(全体员工)</el-checkbox>
				  </el-form-item>
				</el-col>
			</el-row>
			<el-row>
			  <h5>
			    <i class="el-icon-files"></i>发布文件
			  </h5>
			</el-row>
			<el-row>
			  <el-col :span="24">
			    <global-uploader></global-uploader>
			    <el-button size="small" type="primary" @click="tapUpload" title="选择需要上传的文件">选择文件</el-button>
			    <el-button
			      size="small"
			      type="danger"
			      @click="tapDelAll"
			...
			<el-row>
			  <el-col :span="6">
			    <el-form-item
			      label="发文单位："
			      class="te-item"
			      prop="flowBaseData.text10"
			      :rules="[{ required: true, message: '发文单位不能为空',trigger:'blur'}]"
			    >
			  			<el-input v-model="flowData.flowCoreData.text10" clearable></el-input>
			    </el-form-item>
			  </el-col>
			  <el-col :span="6">
			    <el-form-item
			      label="主送单位："
			      class="te-item"
			      prop="flowBaseData.text11"
			      :rules="[{ required: true, message: '主送单位不能为空',trigger:'blur'}]"
			    >
			  			<el-input v-model="flowData.flowCoreData.text11" clearable></el-input>
			    </el-form-item>
			  </el-col>
			  <el-col :span="6">
			    <el-form-item
			      label="签发人："
			      class="te-item"
			      prop="flowBaseData.text12"
			      :rules="[{ required: true, message: '不能为空',trigger:'blur'}]"
			    >
			  			<el-input v-model="flowData.flowCoreData.text12" clearable></el-input>
			    </el-form-item>
			  </el-col>
			  <el-col :span="6">
			    <el-form-item
			      label="发布时间："
			      class="te-item"
			      prop="flowCoreData.text13"
			      :rules="[{ required: true, message: '发布时间不能为空',trigger:'blur'}]"
			    >
			      <el-date-picker v-model="flowData.flowCoreData.text13" type="date" placeholder="选择发布时间" value-format="yyyy-MM-dd" style="width: 100%;"></el-date-picker>
			    </el-form-item>
			  </el-col>
			</el-row>
            <el-row>
...
import qs from "qs";
import Bus2 from "@assets/js/bus.js";
let _self = null;
...
export default {
  name: "flow-flowaddpush-TFmWF_ComDocPostTemp",
  mixins: [myMixin],
  components: {
	...
	tapDelAll() {
	  ...
	  that
	    ...
	    .then(() => {
	      ...
	      that.$message({ message: "删除全部成功!", type: "success" });
	    });
	},
    //梳理数据
    filter(par) {
