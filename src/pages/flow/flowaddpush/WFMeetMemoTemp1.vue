        ...
        <el-col :span="24">
          <el-form :model="flowData" ref="flowData" label-position="left" label-width="110px" class="demo-ruleForm" size="mini">
            ...
            <el-row style="margin-bottom: 18px">
             ...
             <el-row>
             	<el-col :span="16">
             		<el-form-item label="会议主题" class="te-item" prop="flowCoreData.text1" :rules="[{ required: true, message: '会议主题不能为空',trigger:'blur'}]">
             			<deng-select-meet :value="flowData.flowCoreData.text1" @selectOK="selectOK" :readonly="(Flg=='VIEW')"/>
             		</el-form-item>
             	</el-col>
             	<el-col :span="8">
             		<el-form-item label="会议类别" class="te-item" prop="flowCoreData.text2" :rules="[{ required: true, message: '会议类别不能为空',trigger:'blur'}]">
             			<el-select v-model="flowData.flowCoreData.text2" placeholder="请选择" style="width: 100%" :disabled="(Flg=='VIEW')">
             			  <el-option label="公司会议" value="公司会议"/>
						  <el-option label="部门会议" value="部门会议"/>
						  <el-option label="项目会议" value="项目会议"/>
						  <el-option label="招标会" value="招标会"/>
						  <el-option label="专题会议" value="专题会议"/>
						  <el-option label="周例会议" value="周例会议"/>
						  <el-option label="其它会议" value="其它会议"/>
             			</el-select>
             		</el-form-item>
             	</el-col>
             </el-row>
			 <el-row>
			   <el-col :span="8">
			     <el-form-item label="主持人：" class="te-item" prop="flowCoreData.hrsource1" :rules="[{ required: true, message: '主持人不能为空',trigger:'blur'}]">
			       <deng-select-user :value="flowData.flowCoreData.hrsource1_V" :canMark="'one'" :CYUserSelectValue="flowData.flowCoreData.CYUserSelectValue_hrsource1||[]" @submitdata="sethrs($event,'hrsource1')" :readonly="(Flg=='VIEW')"/>
			     </el-form-item>
			   </el-col>
			   <el-col :span="16">
			     <el-form-item label="出席人：" class="te-item" prop="flowCoreData.hrs1" :rules="[{ required: true, message: '出席人不能为空',trigger:'blur'}]">
			       <deng-select-user :value="flowData.flowCoreData.hrs1_V" :CYUserSelectValue="flowData.flowCoreData.CYUserSelectValue_hrs1||[]" @submitdata="sethrs($event,'hrs1')" :readonly="(Flg=='VIEW')"/>
			     </el-form-item>
			   </el-col>
			 </el-row>
			 <el-row>
			   <el-col :span="24">
			     <el-form-item label="抄送人：" class="te-item" prop="flowCoreData.hrs2" :rules="[{ required: true, message: '抄送人不能为空',trigger:'blur'}]">
			       <deng-select-user :value="flowData.flowCoreData.hrs2_V" :CYUserSelectValue="flowData.flowCoreData.CYUserSelectValue_hrs2||[]" @submitdata="sethrs($event,'hrs2')" :readonly="(Flg=='VIEW')"/>
			     </el-form-item>
			   </el-col>
			 </el-row>
			 <el-row>
			   <el-col :span="12">
			     <el-form-item label="会议地点：" class="te-item" prop="flowCoreData.text6" :rules="[{ required: true, message: '抄送人不能为空',trigger:'blur'}]">
			       <el-input v-model="flowData.flowCoreData.text6" :readonly="(Flg=='VIEW')" clearable></el-input>
			     </el-form-item>
			   </el-col>
			   <el-col :span="12">
			     <el-form-item label="会议时间：" class="te-item" prop="flowCoreData.text7" :rules="[{ required: true, message: '抄送人不能为空',trigger:'blur'}]">
			       <el-date-picker style="width:100%;" v-model="flowData.flowCoreData.text7" :readonly="(Flg=='VIEW')" type="date" placeholder="会议时间"/>
			     </el-form-item>
			   </el-col>
			 </el-row>
			 <el-row>
			   <el-col :span="12">
			     <el-form-item label="会议编号：" class="te-item" prop="flowCoreData.text8" :rules="[{ required: true, message: '抄送人不能为空',trigger:'blur'}]">
			       <el-input v-model="flowData.flowCoreData.text8" :readonly="(Flg=='VIEW')" clearable></el-input>
			     </el-form-item>
			   </el-col>
			   <el-col :span="12">
			     <el-form-item label="会议密级：" class="te-item" prop="flowCoreData.text9" :rules="[{ required: true, message: '抄送人不能为空',trigger:'blur'}]">
			       <el-select v-model="flowData.flowCoreData.text9" :disabled="(Flg=='VIEW')" placeholder="请选择" style="width: 100%">
			 ...
			 <el-row>
			   <el-col :span="24">
			     <el-form-item label="具体内容：" class="te-item">
			       <el-input type="textarea" placeholder="请输入会议纪要具体内容..." :autosize="{ minRows: 4}" :minlength="5" v-model="flowData.flowCoreData.text10" maxlength="200" :readonly="(Flg=='VIEW')" show-word-limit></el-input>
			     </el-form-item>
			   </el-col>
			 </el-row>
            <!-- 加载流程其它数据模块 -->
...
export default {
  name: "flow-flowaddpush-WFMeetMemoTemp1",
  mixins: [myMixin],
  components: {
	...
	'dengSelectMeet': () => import(`@cps/deng-select-meet`),
  },
  data() {
    return {
		tagName:"",
		...
		flowData: {
			flowBaseData: {},
			flowCoreData: { CYUserSelectValue_hrsource1:[],CYUserSelectValue_hrs1:[],CYUserSelectValue_hrs2:[],hrs_V: "",text1:"" },
			relaFlowObj: [],
			annexData: [],
			agree: "同意",
			opinionData: "",
  ...
  methods: {
    ...
    sethrs(users,HRMark) {
      ...
      this.flowData.flowCoreData[HRMark] = '@' + tmpArr.join(',')
	...
	selectOK(item){
		let that = this
		console.log('selectOK=>',item)
		that.flowData.flowCoreData.text1 = item.MeetTitle;
		that.flowData.flowCoreData.text2 = item.MeetType;
		that.flowData.flowCoreData.hrsource1 = item.LeadingMan;//主持人
		that.flowData.flowCoreData.hrsource1_V = item.LeadName;//主持人
		that.flowData.flowCoreData.hrs1 = item.JoinManGUID;//出席人
		that.flowData.flowCoreData.hrs1_V = item.JoinMan;//出席人
		that.flowData.flowCoreData.hrs2 = item.JoinManGUID;//抄送人 默认置为出席人
		that.flowData.flowCoreData.hrs2_V = item.JoinMan;//抄送人 默认置为出席人
		that.flowData.flowCoreData.text6 = item.MeetName;// item.MeetAddr;
		that.flowData.flowCoreData.text7 = item.MeetDate;
		//that.flowData.flowCoreData.text8 = "";//会议编号
		//that.flowData.flowCoreData.text9 = "";//会议密级
	}
  }
}
</script>
