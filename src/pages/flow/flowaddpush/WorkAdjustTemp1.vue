      ...
      <el-row>
        <el-col :span="24">
          <el-form :model="flowData" ref="flowData" label-position="left" label-width="110px" class="demo-ruleForm" size="mini">
            ...
            <el-row>
              <el-col :span="6">
                <el-form-item label="计划日期：" class="te-item" prop="flowCoreData.date3" :rules="[{ required: true, message: '还款日期不能为空',trigger:'blur'}]">
                  <el-date-picker style="width:100%;" v-model="flowData.flowCoreData.date3" :readonly="(Flg=='VIEW')" type="date" placeholder="请选择"/>
                </el-form-item>
              </el-col>
			  <el-col :span="10">
			    <el-form-item label="计划等级：" class="te-item" prop="flowCoreData.text3" :rules="[{ required: true, message: '计划等级不能为空',trigger:'blur'}]">
			      <el-radio-group v-model="flowData.flowCoreData.text3" :disabled="(Flg=='VIEW')">
			        <el-radio label="里程碑"></el-radio>
			...
			<el-row>
				<el-col :span="24">
				  <el-form-item label="调整事项：" class="te-item" prop="flowCoreData.text8" :rules="[{ required: true, message: '调整事项不能为空',trigger:'blur'}]">
				    <el-input type="text" v-model="flowData.flowCoreData.text8" placeholder="调整事项..." :readonly="(Flg=='VIEW')" clearable></el-input>
				  </el-form-item>
				</el-col>
			 ...
			 <el-row>
			<el-col :span="6">
			  <el-form-item label="计划类型：" class="te-item" prop="flowCoreData.text20" :rules="[{ required: true, message: '计划类型不能为空',trigger:'blur'}]">
			    <el-input placeholder="请输入计划类型" v-model="flowData.flowCoreData.text20" :maxlength="45" autocomplete="off" :readonly="(Flg=='VIEW')" clearable></el-input>
			  </el-form-item>
			</el-col>
			<el-col :span="12">
			  <el-form-item label="调整类型：" class="te-item" prop="flowCoreData.text4" :rules="[{ required: true, message: '',trigger:'blur'}]">
			    <el-radio-group v-model="flowData.flowCoreData.text4" :disabled="(Flg=='VIEW')">
			      <el-radio label="取消计划"></el-radio>
			      <el-radio label="调整计划"></el-radio>
			      <el-radio label="关闭计划"></el-radio>
			    </el-radio-group>
			  </el-form-item>
			</el-col>
			  </el-row>
			 <el-row>
				 <el-col :span="6">
				   <el-form-item label="调整日期：" class="te-item" prop="flowCoreData.date2" :rules="[{ required: true, message: '输入调整后日期',trigger:'blur'}]">
				     <el-date-picker style="width:100%;" v-model="flowData.flowCoreData.date2" :readonly="(Flg=='VIEW')" type="date" placeholder="请选择"/>
				   </el-form-item>
				 </el-col>
				 
			  <el-col :span="12">
			    <el-form-item label="计划部确认：" class="te-item" prop="flowCoreData.text5" :rules="[{ required: true, message: '',trigger:'blur'}]">
			      <el-radio-group v-model="flowData.flowCoreData.text5" :disabled="(Flg=='VIEW')">
			        <el-radio label="同意调整"></el-radio>
			        <el-radio label="不同意调整"></el-radio>
			      </el-radio-group>
			    </el-form-item>
			  </el-col>
			 </el-row>
			 <el-row>
			   <el-col :span="24">
			     <el-form-item label="调整原因：" class="te-item" prop="flowCoreData.text7" :rules="[{ required: true, message: '调整原因不能为空',trigger:'blur'}]">
			       <el-input type="textarea" :autosize="{ minRows: 6}" v-model="flowData.flowCoreData.text7" placeholder="请输入调整原因..." :readonly="(Flg=='VIEW')" clearable></el-input>
			     </el-form-item>
			   </el-col>
			 </el-row>
			 <el-row>
...
export default {
  ...
  name: "flow-flowaddpush-WorkAdjustTemp1",
  mixins: [myMixin],
  components: {
  ...
  data() {
    return {
		...
		newColumnValue: {
		  //GUID:util.newGUID(),
		  //MGUID:"121",
		  Text11:"",text5:"",date2:"",text1:"",amt3:"",amt4:"",amt2:"",amt5:"",amt1:""
		},
		// 校检规则
		rules: {
		...
		columns: [
		  ...
		  },
		  {
			...
			content: {
			  ...
			  type: "el-date-picker",
			  // attrs 是组件的属性  type="date" placeholder="选择结束日期" value-format="yyyy-MM-dd" style="width: 10px;"
			  attrs: {
			    type: "date",
			    placeholder: "选择日期",
