      ...
      <el-row>
        <el-col :span="24">
          <el-form :model="flowData" ref="flowData" label-position="left" label-width="110px" class="demo-ruleForm" size="mini">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="上级领导：" class="te-item" prop="flowCoreData.hrsource1" :rules="[{ required: true, message: '相关审批人不能为空',trigger:'blur'}]">
                  <deng-select-user :value="flowData.flowCoreData.hrsource1_V" :canMark="'one'" :CYUserSelectValue="flowData.flowCoreData.CYUserSelectValue_hrsource1||[]" @submitdata="sethrs($event,'hrsource1')"/>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="24" class="te-item">
                <template>
                  <ele-table-editor
                    ...
                    v-model="WF_WorkFlowContentD__attent"
                  />
                </template>
              </el-col>
            </el-row>
...
let _self = null;
export default {
  name: "flow-flowaddpush-AttentProblemTemp1",
  mixins: [myMixin],
  components: {
  ...
  data() {
    return {
		...
		//dullTableCof:[{tabName:"WF_WorkFlowContentD__attent",fileds:["MGUID","Workflowguid"]}],
		flowData: {
			flowBaseData: {},
    ...
    if (that.Flg) {
      if ("EDT" == that.Flg||"VIEW" == that.Flg) {
        ...
        that.OpenFlow({ FlowGUID: that.FlowGUID }).then((res) => {
		  ...
		  //取表的数据
		  that.WF_WorkFlowContentD__attent = that.flowData.flowTabsData.WF_WorkFlowContentD__attent
        });
      }
    } else {
      //初始化页面新建模板数据
