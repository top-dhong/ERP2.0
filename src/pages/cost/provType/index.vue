        ...
        <el-card shadow="always" style="margin-left: 5px;">
          ...
          >
			...
			<el-form-item
			  label="分类号"
			  prop="TypeNo"
			  :rules="[{ required: true, message: '分类号不能为空',trigger:'blur'}]"
			>
			  <el-input
			    v-model="menuform.TypeNo"
			    placeholder="请输入分类号"
				clearable
			  ></el-input>
			</el-form-item>
...
import {GetprovTypelist} from "@/api/cost/cost";
...
export default {
  ...
  methods: {
    initData() {
		let that = this
		that.loading = true
		GetprovTypelist().then(async res => {
			that.loading = false
    ...
    del() {
		...
		cancelButtonText: "取消"
			...
			let param = {tabName:'CT_ProvType',GUID:that.currentId};
    ...
    submit() {
		...
		that.$refs.menuform.validate((valid) => {
			...
			if(valid){
				that.loaging2 = true
				SaveAndUpTab(qs.stringify({tabName:'CT_ProvType',jsonStr:JSON.stringify(jsonStr)})).then(async res => {
					that.loaging2 = false
					if(typeof(res)!='undefined'&&res.code==0){
						that.$message({message: res.msg,type: 'success'})
						setTimeout(function(){that.initData();},1500)
	...
	localEdttor(data){
		let that = this
		that.menuList.forEach(function(item,idx){
